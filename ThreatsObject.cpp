    ...
    else
    {
        if (on_ground_ == true)
        {
            if (x_pos_ > animation_b_)
            {
                ...
                LoadImage("IMG/threat_left.png", screen);
            }
            else if (x_pos_ < animation_a_)
            {
                input_type_.left_ = 0;
                input_type_.right_ = 1;
                LoadImage("IMG/threat_right.png", screen);
            }
        }
        else
        {
            if (input_type_.left_ == 1)
            {
                LoadImage("/threat_left.png", screen);
            }
        }
    }
}

void ThreatsObject::InitBullet(BulletObject* p_bullet, SDL_Renderer* screen, MainObject*p_player)
{
    if (p_bullet != NULL)
    {
        ...
        if (ret)
        {
            ...
            int status = p_player->get_status();
          //  std::cout << "Player status: " << status << std::endl;
            if (status == MainObject::WALK_LEFT)
            {
                p_bullet->set_x_val(-5);
            }
            else if (status == MainObject::WALK_RIGHT)
            {
